...
it('write_vm_count: should print the messageId of the Pub/Sub message', done => {
  ...
  const logs = childProcess
    .execSync(`gcloud beta functions logs read write_vm_count --execution-id="${messageId}"`)
    .toString();
  ...
  // Check to see that 1 Pub/Sub message has been sent
  var regexp = /Sent pubsub message/g;
  var messageCount = logs.toString().match(regexp); 
  if (messageCount== null) {
    messageCount = [];
  }
  assert.strictEqual(messageCount.length,1);

  done();
});


it('write_vm_count: should print the error message when no input is sent in Pub/Sub trigger message ', done => {
...
it('write_vm_count: should print the error message when token doesnt match config ', done => {
